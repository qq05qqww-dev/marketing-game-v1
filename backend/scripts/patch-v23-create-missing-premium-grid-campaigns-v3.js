/**
 * V2.3 第 30 批修補腳本 v3
 * 檔名：patch-v23-create-missing-premium-grid-campaigns-v3.js
 *
 * 目的：
 * 1. 確認每個商家都有 GRID / 精緻九宮格活動
 * 2. 確認每個 GRID 活動都有可抽獎品 Prize
 * 3. 解決前台九宮格「跑到一半停止、沒有跳出中獎結果」的常見原因：
 *    後端 DrawEngine 找不到該 GRID 活動可抽的 Prize
 *
 * 使用方式：
 * 1. 放到：
 *    backend/scripts/patch-v23-create-missing-premium-grid-campaigns-v3.js
 *
 * 2. 在 backend 目錄執行：
 *    node scripts/patch-v23-create-missing-premium-grid-campaigns-v3.js
 *
 * 3. 跑完後重開後端與前端
 */
...
const ensurePrizesForTenantCampaign = async (tenant, campaign) => {
  ...
  if (!Object.keys(where).length) {
    log('   ↳ Prize model 找不到 campaignId / tenantId 可查詢欄位，略過獎品補建。')
    return
  }

  const existingCount = await prisma.prize.count({ where })

  if (existingCount > 0) {
    log(`   ↳ 已有 ${existingCount} 個獎品，略過獎品補建。`)
    return
  }
  ...
  log(`   ↳ 已補建 ${DEFAULT_GRID_PRIZES.length} 個精緻九宮格預設獎品。`)
}

const main = async () => {
  log('')
  log('==============================================')
  log('V2.3 補建精緻九宮格 GRID 活動與獎品腳本 v3 開始')
  log('==============================================')
  log('')
  ...
  if (!tenants.length) {
    log('⚠️ 目前沒有任何商家 Tenant，因此沒有建立 GRID 活動與獎品。')
    return
  }
...
try {
  ...
  console.error('')
  console.error('❌ 補建精緻九宮格 GRID 活動與獎品失敗')
  console.error(error)
  console.error('')
  process.exitCode = 1
} finally {
