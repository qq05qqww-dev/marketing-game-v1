...
const hasRequiredCreateFieldsMissing = (modelName, data) => {
  ...
  return fields
    .filter((field) => {
      ...
      if (ignored.has(field.name)) return false

      // Prisma runtimeDataModel 會把 relation list 也標成 required，
      // 但 create Campaign 時不需要直接填 prizes / rewards / playRecords 這類關聯陣列。
