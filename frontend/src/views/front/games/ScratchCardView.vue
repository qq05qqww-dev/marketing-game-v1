...
const PREMIUM_SCRATCH_SYNC_EVENT = 'premium-scratch-state-updated'
const PREMIUM_SCRATCH_SYNC_PING_KEY = 'premium-scratch-sync-ping'
const isApplyingPremiumScratchRemoteState = ref(false)
const isSavingPremiumScratchState = ref(false)
...
const scratchResultLabel = computed(() => {
  if (!hasScratched.value || !resultPrize.value) return ''

  return resultPrize.value.type === 'lose' ? '未中獎結果' : '恭喜中獎'
})

const scratchResultClass = computed(() => {
  if (!hasScratched.value || !resultPrize.value) return 'bg-slate-100 text-slate-500'
...
const resultActionText = computed(() => {
  ...
  if (player.chances > 0) return '繼續刮卡'
...
const resultHintText = computed(() => {
  ...
  if (player.chances > 0) {
    return `結果已寫入我的遊戲紀錄，目前還有 ${player.chances} 次刮卡機會。`
  }

  return '結果已寫入我的遊戲紀錄，目前刮卡機會已用完，可分享活動增加 1 次。'
})
...
const scratchingHintText = computed(() => {
  if (!isScratching.value) return ''

  return '刮卡進行中，請不要重複點擊或關閉畫面。'
})

const playerStatusMessage = computed(() => {
  if (isScratching.value) return '正在刮開獎區，系統會自動顯示結果。'
...
const getShareSuccessText = (prefix = '已增加 1 次刮卡機會') => {
  return `${prefix}，目前還有 ${player.chances} 次刮卡機會。`
}
...
const getPremiumScratchStorageKey = () => {
  return `${PREMIUM_SCRATCH_STORAGE_KEY}_${currentGameId.value}`
}

const broadcastPremiumScratchStateChange = () => {
  // 不再對同一個頁面發送自訂事件，避免 watch -> save -> load -> save 形成迴圈。
  // 不同分頁會透過瀏覽器 storage 事件同步。
}

const pingPremiumScratchSync = () => {
  if (typeof localStorage === 'undefined') return

  localStorage.setItem(
    PREMIUM_SCRATCH_SYNC_PING_KEY,
    JSON.stringify({
      key: getPremiumScratchStorageKey(),
      gameId: currentGameId.value,
...
const syncPremiumScratchToPlayer = () => {
  savePremiumScratchState()
  pingPremiumScratchSync()

  lastSyncAt.value = new Date().toLocaleString('zh-TW')
  lastSyncMessage.value = `已同步到玩家版：${sourcePath.value}`

  showSavedMessage('已同步最新刮刮卡設定到玩家版。')
}

const handlePremiumScratchStorageSync = (event) => {
  if (!event) return
  if (isSavingPremiumScratchState.value) return

  if (event.key === getPremiumScratchStorageKey()) {
    loadPremiumScratchState()
    return
  }

  if (event.key === PREMIUM_SCRATCH_SYNC_PING_KEY) {
    const payload = safeJsonParse(event.newValue, null)

    if (payload?.key === getPremiumScratchStorageKey()) {
      lastSyncAt.value = new Date().toLocaleString('zh-TW')
...
const handlePremiumScratchCustomSync = (event) => {
  if (!event?.detail || event.detail.key !== getPremiumScratchStorageKey()) return
  if (isSavingPremiumScratchState.value) return
...
const savePremiumScratchState = () => {
  if (isApplyingPremiumScratchRemoteState.value) return

  isSavingPremiumScratchState.value = true
  ...
  localStorage.setItem(getPremiumScratchStorageKey(), JSON.stringify(payload))
  pingPremiumScratchSync()

  setTimeout(() => {
    isSavingPremiumScratchState.value = false
  }, 0)
}

const loadPremiumScratchState = () => {
  isApplyingPremiumScratchRemoteState.value = true

  const payload = safeJsonParse(
    localStorage.getItem(getPremiumScratchStorageKey()),
    null
  )

  if (!payload) {
    updateChanceText()
    isApplyingPremiumScratchRemoteState.value = false
    return
  }
...
const shareCampaign = async () => {
  ...
  try {
    ...
    showShareSuccess(getShareSuccessText('已複製分享文案並增加 1 次刮卡機會'))
  } catch (error) {
    console.error('複製分享文案失敗：', error)
    showShareSuccess(getShareSuccessText('已增加 1 次刮卡機會；目前瀏覽器不支援自動複製'))
  }
}
...
const resetDemo = () => {
  ...
  localStorage.removeItem(getPremiumScratchStorageKey())
...
onMounted(() => {
  ...
  window.addEventListener('storage', handlePremiumScratchStorageSync)
})

onBeforeUnmount(() => {
  if (typeof window === 'undefined') return

  window.removeEventListener('storage', handlePremiumScratchStorageSync)
})
        ...
        <div class="mt-5 rounded-3xl border border-slate-200 bg-slate-50 p-4">
          ...
          <div class="mt-3 space-y-3">
            ...
            >
              ...
              <p
                class="mt-2 inline-flex rounded-full px-2.5 py-1 text-[11px] font-black"
                :class="prize.type === 'lose'
                  ? 'bg-slate-100 text-slate-500'
                  : 'bg-orange-100 text-orange-700'
                "
              >
                目前類型：{{ getPrizeTypeLabel(prize.type) }}
              </p>
            </article>
          </div>
        </div>
            ...
            <div class="relative">
              ...
              <section class="mt-5 rounded-3xl border border-white/25 bg-white/15 p-4 text-center shadow-inner backdrop-blur">
                ...
                <p
                  ...
                  class="mx-auto mt-2 inline-flex rounded-full bg-white/20 px-3 py-1 text-[11px] font-black text-white/80"
                >
              ...
              <section
                class="relative mt-6 rounded-[30px] border-[5px] border-yellow-300/70 bg-gradient-to-br from-yellow-100 to-orange-100 p-4 shadow-[0_24px_40px_rgba(154,52,18,.35)]"
                :class="isScratching ? 'premium-scratch-active' : ''"
              >
                <div class="rounded-[26px] bg-white p-4 text-center text-slate-900 shadow-inner">
                  <div
                    v-if="scratchResultLabel"
                    class="mx-auto mb-3 inline-flex rounded-full px-4 py-2 text-xs font-black"
                    :class="scratchResultClass"
                  >
                    {{ scratchResultLabel }}
                  </div>
                  ...
                  <p class="mt-2 text-xs font-black text-slate-400">
                    {{ hasScratched ? '刮卡完成，結果已產生' : `刮開進度 ${scratchProgress}%` }}
                  </p>
              ...
              <section class="relative mt-4 grid gap-3 sm:grid-cols-2">
                <button
                  ...
                  :class="isScratching ? 'cursor-not-allowed opacity-60' : ''"
    ...
    >
      <div class="w-full max-w-sm overflow-hidden rounded-[32px] bg-white shadow-2xl">
        ...
        <div class="p-6 text-center">
          ...
          <p class="mt-3 rounded-2xl bg-orange-50 px-4 py-3 text-xs font-black leading-5 text-orange-700">
            {{ resultHintText }}
          </p>
          ...
          <div class="mt-5 grid gap-3">
            <button
              type="button"
              class="w-full rounded-2xl px-5 py-3 text-sm font-black text-white transition"
...
.premium-scratch-active {
  animation: premium-scratch-pulse 0.8s ease-in-out infinite alternate;
}

@keyframes premium-scratch-pulse {
  from {
    filter: brightness(1);
  }
